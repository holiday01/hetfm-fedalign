...
OUT = Path("/home/holiday01/fl_wsi_adaptive/manuscript_hetfm/figures")
...
def fig1():
    ...
    ax.text(7.85, 3.1, "Server\nshared head $g_\\phi$\n+ global class\n"
            "prototypes $\\mu_c$", ha="center", va="center", fontsize=10.5)
...
def fig2():
    ...
    ax.set_ylim(0, 0.8)
    ax.annotate(f"gap-closure {clo:.2f}\n($\\geq$0.90 pre-registered)",
                xy=(3, m["mean"]), xytext=(1.5, 0.7), fontsize=10.5,
                arrowprops=dict(arrowstyle="->", color="#444"))
    ax.set_title(f"Pre-registered confirmation, $n=${pr['n']} fresh seeds",
                 fontsize=12)
    save(fig, "fig2_rq1_recovery.pdf")


# ---- Fig 3: RQ4 assignment-skew inversion (exploratory n=5) -----------------
def fig3():
    s = J["week6_rq4"]["schemes"]
    ...
    for i, v in enumerate(mm):
        ax.text(i, v + me[i] + 0.015, f"{v:.3f}", ha="center", fontsize=10.5)
    ...
    ax.set_title("Exploratory, $n=5$ seeds (RQ4 not pre-registered)",
                 fontsize=12)
...
def fig4():
    w5 = J["week5_fedprotokd_B"]["schemes"]
    pr = J["prereg_confirm"]
    schemes = ["balanced", "cancer_correlated", "adversarial"]
    disp = ["balanced\n($n{=}30$ pre-reg)", "cancer-correlated\n($n{=}5$)",
            "adversarial\n($n{=}5$)"]
    meth = [pr["method_linear_balanced"]["mean"],
            w5["cancer_correlated"]["method_linear_REUSED"]["mean"],
            w5["adversarial"]["method_linear_REUSED"]["mean"]]
    fp = [pr["fedprotokd_B_balanced"]["mean"],
          w5["cancer_correlated"]["fedprotokd_B"]["mean"],
          w5["adversarial"]["fedprotokd_B"]["mean"]]
    fig, ax = plt.subplots(figsize=(6.5, 4.2))
    x = range(3); w = 0.36
